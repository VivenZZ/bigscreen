<template>
  <div class="home">
      <div>
        ...
        <div class="btn">
          <div @click="btnFunction(item, index)" :class="currentBtn === index ? 'current': ''" v-for="(item, index) in btn" :key="index">{{item}}</div>
        </div>
      </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.$fetch('https://easy-mock.com/mock/5c7df07ac05ec81060cf5443/test/goodsList')
      .then((response) => {
